...
fn main() {
    ...
    //
    let library = {
        let file = File::open(&Path::new("./examples/list.markup")).unwrap();
        let reader = BufReader::new(file);
        uil::markup::parse(uil::StdOutErrorReporter, reader)
    };

    let defs = uil::deps::parse_file(uil::StdOutErrorReporter, "./examples/list.deps");
    ...
    let display = glutin::WindowBuilder::new()
        .with_vsync()
        .with_title("uil - list example".to_string())
        .build_glium()
        .unwrap();
    ...
    let stylesheet = {
        let file = File::open(&Path::new("./examples/list.style")).unwrap();
        let reader = BufReader::new(file);
